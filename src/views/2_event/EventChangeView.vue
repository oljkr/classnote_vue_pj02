<template>
  <div>
    <select name="" id="" @change="changeCity" v-model="selectedCity">
      ...
      <option :value="city.cityCode" v-for="city in cityList" :key="city.code">{{ city.title }}</option>
    </select>
    <select name="" id="">
      <option :value="dong.dongCode" v-for="dong in selectedDongList" :key="dong.cityCode">{{ dong.dongTitle }}</option>
    </select>
    <hr>
